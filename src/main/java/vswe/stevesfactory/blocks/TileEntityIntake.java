...
public class TileEntityIntake extends TileEntityClusterElement implements IInventory {
    ...
    public void setInventorySlotContents(int id, ItemStack itemstack) {
        ...
        if (id < 0 || !canPickUp(items.get(id))) {
            if (itemstack != null) {
                EnumFacing direction = EnumFacing.getFront(gigabit101.AdvancedSystemManager2.blocks.ModBlocks.blockCableIntake.getSideMeta(getBlockMetadata()) % EnumFacing.values().length);
