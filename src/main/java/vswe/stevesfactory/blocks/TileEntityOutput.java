...
public class TileEntityOutput extends TileEntityClusterElement implements IPacketBlock, IRedstoneNode {
    ...
    private void notifyUpdate(BlockPos pos, boolean spread) {
        if (worldObj.getBlockState(pos).getBlock() != gigabit101.AdvancedSystemManager2.blocks.ModBlocks.blockCable && (pos.getX() != getPos().getX() || pos.getY() != getPos().getY() || pos.getZ() != getPos().getZ())) {
